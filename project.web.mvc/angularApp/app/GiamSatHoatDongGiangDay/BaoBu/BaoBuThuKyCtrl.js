...
app.controller("BaoBuThuKyCtrl", function ($scope, $rootScope, BaoBuThuKySvc, $http, BaseDialog, $translate, $uibModal, $filter, $window) {
    ...
    $scope.NamID = BaoBuThuKySvc.getNamHocHienTai();
    ...
    BaoBuThuKySvc.GetApiCall(BaoBuThuKySvc.API_ListNamHoc)
      .then(function (response) {
          for (var i = 0; i < response.data.length ; i++) {
    ...
    $scope.loadListMon = function () {
        ...
        $scope.listShow = [];
        BaoBuThuKySvc.GetApiCall(BaoBuThuKySvc.API_ListMonShow + "?NhanVienGuid=" + $scope.NhanVienGuid + "&NamID=" + $scope.NamID)
       .then(function (response) {
    ...
    $scope.deletePhieu = function (id) {
        ...
        BaoBuThuKySvc.showloading();
        $scope.listShow = [];
        BaoBuThuKySvc.GetApiCall(BaoBuThuKySvc.API_DeletePhieuBu + "?PhieuGuid=" + id)
       .then(function (response) {
    ...
    $scope.addBuoi = function (Guid) {
        ...
        //var ItemShow = $filter('filter')($scope.listShow, { LopHocGuid: Guid }, true)[0];
        //var ItemShow = $scope.listShow[index];
        ...
        //else {
            ...
            //ItemShow.TietBatDau = '' + ItemShow.TietBatDau + '';
            //ItemShow.row = Math.floor((Math.random() * 1000) + 1);
        //$scope.listAdd.push(listSave);
        //}
