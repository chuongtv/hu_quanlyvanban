...
app.factory('KhoaPhongSvc', function ($q, $http, BaseServices) {
    ...
    service.API_ListPhongTrong = 'api/DuLieuNghiBu/GET_ListKhoaPhongTrong';
    ...
    service.API_ListTinhChatPhong = 'api/ThongTinCaiDatChung/GET_TinhTrangPhong_GetListTinhChatPhong';
    service.API_ListNamHoc = 'api/NamHoc/GET_ListNamHoc';
    service.API_DeletePhieuBu = 'api/DuLieuNghiBu/GET_DeletePhieuKhoaPhong';
    service.API_DeleteDieuChinh = 'api/DuLieuNghiBu/GET_DeleteDieuChinhPhieuKhoaPhong';
...
app.controller("KhoaPhongCtrl", function ($scope, $rootScope, KhoaPhongSvc, $http, BaseDialog, $translate, $uibModal, $filter, $window) {
    ...
    $scope.listThu = [{ name: 'Thứ', value: '1' },
                      { name: '2', value: '2' },
                      { name: '3', value: '3' },
                      { name: '4', value: '4' },
                      { name: '5', value: '5' },
    ...
    $scope.listTinhChat = [{ name: 'Tính chất phòng', value: '' }];
    $scope.Thu = '1';
    $scope.TietBatDau = '0';
    $scope.TietKetThuc = '0';
    $scope.TinhChat = '';
    ...
    KhoaPhongSvc.GetApiCall(KhoaPhongSvc.API_ListTinhChatPhong)
      .then(function (response) {
          for (var i = 0; i < response.data.length ; i++) {
              $scope.listTinhChat.push({ name: "" + response.data[i].Phongname + "", value: "" + response.data[i].TinhChat + "" });
          }
       });
    
    $scope.themPhong = function () {
        if ($scope.PhongID.length == 0) {
            alert('Xin vui lòng nhập phòng học');
            return;
        }
    ...
    if ($scope.idParameter != "") {
       ...
       .then(function (response) {
           ...
           $scope.TinhChat = "" + response.data.TinhChat + "";
           $scope.SucChua = response.data.SucChua;
           $scope.Thu = "" + response.data.Thu + "";
       ...
       .then(function (response) {
           $scope.TuNgay = new Date(response.data[0].TuNgay);
           $scope.DenNgay = new Date(response.data[0].DenNgay);
           $scope.TuNgayCu = new Date(response.data[0].TuNgay);
           $scope.DenNgayCu = new Date(response.data[0].DenNgay);
           $scope.TietBatDau = "" + response.data[0].TietBatDau + "";
           $scope.TietKetThuc = "" + response.data[0].TietKetThuc + "";
           $scope.listAdd = response.data;
           $scope.listShow = response.data;
           $scope.figureOutTodosToDisplay($scope.currentPage, $scope.numPerPage);
       }, function (error) {
    ...
    $scope.loadListPhongTrong = function () {
        ...
        var day1 = new Date($scope.TuNgay);
        var day2 = new Date($scope.DenNgay);
        var dayNow = new Date($scope.minDate);
        if (day1 < dayNow) {
        ...
        if ($scope.TietBatDau != '0' && $scope.TietKetThuc != '0') {
            var bd = parseInt($scope.TietKetThuc);
            var kt = parseInt($scope.TietBatDau);
            if (kt > bd) {
                alert("Tiết kết thúc phải sau tiết bắt đầu.");
        ...
        listSearch = {
            ...
            TietKetThuc: $scope.TietKetThuc,
            Thu: $scope.Thu,
            TinhChat: $scope.TinhChat,
            SucChua: $scope.SucChua
        }

        KhoaPhongSvc.PostApiCall(listSearch, KhoaPhongSvc.API_ListPhongTrong)  //goi api get du lieu
            .then(function (response) {
                $scope.listShow = response.data;
                $scope.figureOutTodosToDisplay($scope.currentPage, $scope.numPerPage);

            }, function (error) {
                console.log('loi load phong trong');
            }).finally(function () {
    ...
    $scope.figureOutTodosToDisplay = function (currentPage, numPerPage) {
        var begin = ((currentPage - 1) * numPerPage);
        var end = begin + numPerPage;
        $scope.filteredTodos = $scope.listShow.slice(begin, end);
    };
    ...
    $scope.deleteBuoi = function (Guid) {
        $scope.Check = 0;
        var ItemAdd = $filter('filter')($scope.listAdd, { PhongID: Guid}, true)[0];
        console.log
        var indexOfArray = $scope.listAdd.indexOf(ItemAdd);
        if (indexOfArray > -1)
    ...
    $scope.addBuoi = function (Guid) {
        var ItemShow = $filter('filter')($scope.listShow, { PhongGuid: Guid}, true)[0];
        var ItemAdd = $filter('filter')($scope.listAdd, { PhongGuid: Guid}, true)[0];
        var indexOfArray = $scope.listAdd.indexOf(ItemAdd);
        if (indexOfArray > -1)
            $scope.listAdd.splice(indexOfArray, 1);
        else {
            ...
            ItemShow.TietBatDau = $scope.TietBatDau;
            ItemShow.TietKetThuc = $scope.TietKetThuc;
            ItemShow.TuNgay = $scope.TuNgay;
            ItemShow.DenNgay = $scope.DenNgay;
    ...
    $scope.checkPhieu = function () {
        ...
        KhoaPhongSvc.PostApiCall(listSave, KhoaPhongSvc.API_PostCheckPhieu)  //goi api get du lieu
            .then(function (response) {
                if (response.data != "")
                {
                    alert('Có phòng đã sử dụng hoặc không tồn tại');
    ...
    $scope.deleteAnhEdit = function () {
        ...
        var day1Cu = new Date($scope.TuNgayCu);
        var day2Cu = new Date($scope.DenNgayCu);


        if (day1.getTime() == day1Cu.getTime() && day2.getTime() == day2Cu.getTime()) {
            if (confirm('Ngày bạn chọn trùng với phiếu cũ. Bạn có muốn xóa phiếu cũ ?')) {
                $scope.deleteAnhDieuChinh(1);
            }
        }
        else
        {
            $scope.deleteAnhDieuChinh(0);
        }
    ...
    $scope.deleteAnhDieuChinh = function (Delete) {
        //console.log($scope.listAdd);
        KhoaPhongSvc.showloading();
        ...
        KhoaPhongSvc.PostApiCall(listSave, KhoaPhongSvc.API_DeleteDieuChinh)  //goi api get du lieu
            .then(function (response) {
                $scope.Edit = 1;
                $scope.listAdd = [];
                $scope.loadListPhongTrong();
        ...
        function CreateAutoCompleteSearchPhong() {
            var options = {
                ...
                select: function (event, ui) {
                    ...
                    //$(valueset).val(ui.item.valuequery);
                    $scope.PhongID = ui.item.valuequery;
                    //alert($(valueset).val());
                    return false;
                },
