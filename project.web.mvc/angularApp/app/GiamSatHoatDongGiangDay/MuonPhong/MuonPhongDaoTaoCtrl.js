...
app.factory('MuonPhongDaoTaoSvc', function ($q, $http, BaseServices) {
    ...
    service.API_PostXacNhanCapPhong_DeletePhieu = 'api/DuLieuNghiBu/POST_DeletePhieuMuonPhong?Phieu=';
...
app.controller("MuonPhongDaoTaoCtrl", function ($scope, $rootScope, MuonPhongDaoTaoSvc, $http, BaseDialog, $translate, $uibModal, $window) {
 ...
 $scope.idParameter = MuonPhongDaoTaoSvc.getParameterByName('id');
    ...
    $scope.deletePhieu = function () {
        MuonPhongDaoTaoSvc.PostApiCall(null, MuonPhongDaoTaoSvc.API_PostXacNhanCapPhong_DeletePhieu + $scope.idParameter)  //goi api get du lieu
               .then(function (response) {
                   if (response.data == "ok") {
                       ...
                       alert("Lỗi: Ngày mượn phòng phải lớn hơn ngày hiện tại");
                   }
               }, function (error) {
                   //console.log('loi gui mail');
               }).finally(function () {
...
app.controller('MuonPhongPhanPhongModalCtrl', function ($scope, $uibModalInstance, $filter, objs, MuonPhongDaoTaoSvc) {
    ...
    //    alert("Cần cấp phòng trước ngày mượn phòng ít nhất một ngày. Vui lòng kiểm tra lại!");
    ...
    $scope.DanhSachPhong = function (objs) {
        ...
        MuonPhongDaoTaoSvc.GetApiCallCustomLink(UrlDomainAPI + "api/ThongTinCaiDatChung/GET_TinhTrangPhong_GetListPhongTrong?coso=" + coso + "&lau=" + lau + "&thoigian=" + valueDate + "&tietbatdau=" + objs.tietbatdau + "&sotiet=" + objs.sotiet + "&phongid=" + objs.phongid + "&tinhchat=" + objs.tinhchat.ValueString)
        .then(function (response) {
                    ...
                    if ($scope.objs[i] != undefined)
                        if (objs.listAdd[j].PhongGuid == $scope.objs[i].PhongGuid
                            ...
                            && parseInt(objs.tietbatdau) + parseInt(objs.sotiet) - 1 > parseInt(objs.listAdd[j].TietBatDau) + parseInt(objs.listAdd[j].SoTiet) - 1)
                              ...
                              //  console.log(objs.tietbatdau + "||" + objs.sotiet);
   ...
   .then(function (response) {
       ...
       if ($scope.listCoSo.length > 0) {
           ...
           .then(function (response) {
               ...
               MuonPhongDaoTaoSvc.GetApiCallCustomLink(UrlDomainAPI + "api/ThongTinCaiDatChung/GET_TinhTrangPhong_GetListTinhChatPhong")
                .then(function (response) {
