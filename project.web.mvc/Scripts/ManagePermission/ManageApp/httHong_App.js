...
function ManageApp_App_ReLoadAjaxPartial(itemguid) {
    BeginWaiting();
...
//Sự kiện tìm kiếm
function ManageApp_App_IsSearch(element) {
    $.removeCookie('pageperManageApp', { path: '/' });
...
function ManageApp_App_EnterEvent(event, btn) {
    if (event.which == 13) {
        $(btn).click();
    }
}
...
function ManageApp_App_OnSaveExit(e, v, f) {
...
function ManageApp_App_DeleteView(element) {
    $.removeCookie('pageperManageApp', { path: '/' });
    var $button = $(element);
...
function ManageApp_App_DeleteMulti(element) {
    $.removeCookie('pageperManageApp', { path: '/' });
    var $button = $(element);
    if ($button.attr('data-otf-confirm'))
        if (!confirm($button.attr('data-otf-confirm')))
