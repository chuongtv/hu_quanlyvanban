...
namespace project.web.mvc.Controllers
{
    ...
    {
        #region contructor
        //dmhoaEntities db = new dmhoaEntities();
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                //kiem tra xem email nay da goi thu xac nhan chua
                //if (user == null || !(await UserManager.IsEmailConfirmedAsync(user.Id)))
                if (user == null)
                {
                    ViewBag.Error = "Tài khoản email này chưa không có trong hệ thống. Vui lòng kiểm tra lại.";
                ...
                IdentityResult result = UserManager.AddPassword(user.Id, pass);

                var message = new MailMessage();
                message.Subject = "[HUTECH] Khôi phục mật mã truy cập hệ thống.";
                //đọc nội dung từ 1 fiel html
                message.Body = PopulateBody(user.UserName, "HUTECH", pass);
               // EmailService.SendMailAsync_ResetPass(model.Email, message.Subject, message.Body);
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    //save log login
                    LoggingService.LogCritical("Account", model.UserName, "Đăng nhập hệ thống", LoggingService.CriticalCategory, false);
        ...
        public ActionResult LogOff()
        {
            ...
            //save log Logout
            LoggingService.LogCritical("Account", User.Identity.GetUserName() + " - " + User.Identity.Name, "Đăng xuất hệ thống", LoggingService.CriticalCategory, false);
            return RedirectToAction("Index", "Home");
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult LogOffNhapHocOnl()
        {
            AuthenticationManager.SignOut();
            ...
            return Redirect(ConfigurationManager.AppSettings["linkbaiVietLogOffNhapHocOnl"]);
        }
